...
function TradingChart(): JSX.Element {
    ...
    const timePeriods: TimePeriod[] = ['15m', '1h', '1d', '1w', '1m', '3m', '1y', '3y'];
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8000/api/market-data?ticker=${ticker}&period=${selectedPeriod}`)
            .then(res => res.json())
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (inputTicker.trim() && inputTicker !== ticker) {
    ...
    const formatChangePercent = (change: number | null, lastPrice: number | null): string => {
    ...
    return (
        <div className="trading-chart-container">
            ...
            <div className="chart-info">
                <div className={`info-item ${priceChange && priceChange >= 0 ? 'positive' : 'negative'}`}>
                ...
                <div className={`info-item ${priceChange && priceChange >= 0 ? 'positive' : 'negative'}`}>
                    ...
                    <span className="value">{formatChangePercent(priceChange, lastPrice)}</span>
                </div>
